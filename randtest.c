...
/* For runs test */ 
static long n1, n2, nruns;
static int prev, currentdiff, runsfirst;

/* For local means test */
static long sum;
static const int BLOCKSIZE = 1024; /* Number of elements wanted per block, default 1024 */
static double x2statistic = 0;
static int num_blocks; /* Number of blocks, and also number of degrees of freedom for Chi square test*/
...
void rt_add(void *buf, int bufl)
{
   ...
   /* Process number for local means test */
   double expmean = binary ? 0.5 : 127.5;
   double expvariance = binary ? 0.25 : (256.0*256.0 -1) / 12;
   sum += c;
   if (totalc % BLOCKSIZE == 0) {   /* End of block */
      x2statistic += BLOCKSIZE * (sum / ((double) BLOCKSIZE) - expmean) * (sum / ((double) BLOCKSIZE) - expmean) / expvariance;
      sum = 0;
      ++num_blocks;
   }
      ...
      /* Update runs state */
      currentdiff = c < (binary ? 0.5 : 127.5); // Check whether it is below or above median
      if (currentdiff) ++n1;  // Increase negatives (elts below median)
      else ++n2;              // Increase positives (elts above median)
      if (runsfirst) runsfirst = FALSE;
      else if (prev != currentdiff) ++nruns;   // Switching from negative to positive or vice versa gives new run
      prev = currentdiff;
      oc <<= 1;
       } while (binary && (++bean < 8));
    }
